<template>
  <div class="min-h-screen flex flex-col">
    ...
    <!-- <div class="flex justify-end p-6">
      <router-link to="/" class="flex items-center px-4 py-2 rounded-lg border border-gray-200 bg-gray-50 hover:bg-gray-100 text-gray-700 font-medium shadow-sm transition">
    ...
    <div class="flex flex-col md:flex-row gap-8 w-full flex-1 items-start px-6 mt-40 md:px-16">
      ...
      <div class="w-full max-w-md mx-auto">
        <div  class="bg-[#fffceb] rounded-2xl shadow-lg border border-gray-100 p-8 mb-8">
          ...
          <p class="text-gray-500 mb-6 text-sm">Enter a URL and configure test settings to analyze website performance</p>
          <form @submit.prevent="onFormSubmit">
            <!-- Website URL -->
            <label class="block text-left text-sm font-medium text-gray-700 mb-1" for="url">Website URL</label>
            <input id="url" type="url" v-model="url" placeholder="https://example.com" class="mb-4 w-full px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#fff599] text-gray-900 bg-[#fffceb]" />

            <!-- Device Type -->
            <label class="block text-left text-sm font-medium text-gray-700 mb-1" for="device">Device Type</label>
            <select id="device" v-model="device" class="mb-4 w-full px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#fff599] text-gray-900 bg-[#fffceb]">
              <option>Desktop</option>
              <option>Mobile</option>
            </select>

            <!-- Network Throttling -->
            <label class="block text-left text-sm font-medium text-gray-700 mb-1" for="throttle">Network Throttling</label>
            <select id="throttle" v-model="throttle" class="mb-4 w-full px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#fff599] text-gray-900 bg-[#fffceb]">
            ...
            <input id="runs" type="number" min="1" max="10" v-model="runs" class="mb-1 w-full px-3 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-[#fff599] text-gray-900 bg-[#fffceb]" />
            ...
            >
              ...
              <div class="mr-2" role="status">
                  <svg aria-hidden="true" class="w-5 h-5 text-gray-400 animate-spin fill-gray-900" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <div
      v-if="result.length"
      class="min-h-screen w-full flex flex-col justify-center px-4 py-12 text-center rounded-lg"
      ref="resultsSection"
    >
      <ResultWrapper
        :rawResults="result"
...
import { getLighthouseReport } from "../utils/urlRunner.js";
import { lighthouseRequestUrl } from "../utils/constants.js";
